...
class SimulatedObjectRepresentationStrategy extends JavaObjectRepresentationStrategy implements IObjectRepresentationStrategy {
	...
	override init(JavaReflectAccess reflectAccess, ClassFinder classFinder, IClassManager classManager, XtendInterpreter interpreter) {
		super.init(reflectAccess, classFinder, classManager, interpreter)
		createCaches.clear
	...
	override isInstanceOf(Object obj, String typeFQN) {
		if (obj instanceof XtendObject){
			if (obj.qualifiedClassName == typeFQN){
				return true
			}else{
				//TODO: check superclasses and interfaces
				return false
			}
		}else{
